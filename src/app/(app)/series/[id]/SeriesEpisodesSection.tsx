  ...
  const seasonEpisodesToMark = useMemo(
    () =>
      selectedSeason?.episodes.filter(
        (episode) => !episode.watched && !togglingWatched.has(episode.id)
      ) ?? [],
    [selectedSeason, togglingWatched]
  );
  const isSeasonActionPending = useMemo(
    () =>
      selectedSeason?.episodes.some((episode) => togglingWatched.has(episode.id)) ??
      false,
    [selectedSeason, togglingWatched]
  );
